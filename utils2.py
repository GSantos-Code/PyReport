...
class Book(main.PyReport):
        def __init__(self, width, height, bg, padx):
                ...
                self.Y= 400
                self.tam= 35
                self.fonte= ImageFont.truetype(r"C:\Program Files\PyReport-1900x1080\build\exe.win-amd64-3.9\fonts\Arial.ttf",self.tam)
                self.padx= padx
                self.height= height
        ...
        def Paragraph(self,txt):
        		...
        		for i in txt:
        				...
        				if(i == "<break>" or i == "\n"):
        						...
        						self.fonte= ImageFont.truetype("C:\\Program Files\\PyReport-1900x1080\\build\\exe.win-amd64-3.9\\fonts\\Arial-Bold.ttf",self.tam)
        				elif(i == ">"):
        						self.fonte= ImageFont.truetype("C:\\Program Files\\PyReport-1900x1080\\build\\exe.win-amd64-3.9\\fonts\\Arial.ttf",self.tam)
        				else:
        						self.draw.text((cont, self.Y),i,fill= (0,0,0),font= self.fonte)
        						print(cont, cursor)
        						cont= cont + cursor[0] + 10
        def Textc(self,txt,color,c,tam):
                fonte= ImageFont.truetype("C:\\Program Files\\PyReport-1900x1080\\build\\exe.win-amd64-3.9\\fonts\\Arial.ttf",tam)
                if("c" in c):
